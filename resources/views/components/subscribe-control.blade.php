    ...
    <div x-data="{ syncing: false }" wire:key="subscribe-control-off">
        ...
        >
            <div class="relative flex items-center justify-center">
                <flux:icon.heart x-bind:class="syncing && 'opacity-0'" class="size-5 shrink-0 sm:size-4" />
...
@else
    <flux:dropdown align="start" wire:key="subscribe-control-on">
        ...
        >
            <flux:icon
                :name="$mode->icon()"
                @class(['size-5 shrink-0 sm:size-4', 'fill-current' => $mode->isFilled()])
            />
        ...
        <flux:menu>
            @foreach ($modes as $option)
                ...
                >
                    <span class="flex items-center">
                        <flux:icon
                            :name="$option->icon()"
                            @class(['me-2 size-5 shrink-0', 'fill-current' => $option->isFilled()])
                        />
                        {{ $option->menuLabel() }}
                    </span>
                </flux:menu.item>
            ...
            <flux:menu.item variant="danger" wire:click="unsubscribe">
                <span class="flex items-center">
                    <flux:icon name="heart" class="me-2 size-5 shrink-0" />
                    Unsubscribe
                </span>
            </flux:menu.item>
        </flux:menu>
