...
<ui-modal {{ $attributes }} data-flux-modal>
    ...
    <dialog
        wire:ignore.self
        {{-- This needs to be here because the dialog element adds a "close" attribute that isn't durable... --}}
        ...
        @isset($__livewire)
            ...
            x-on:modal-close.document="
                ...
                if (
                    ! $event.detail.name ||
                    ($event.detail.name === @js($name) && ! $event.detail.scope)
                )
                    $el.close()
            "
        @else
    ...
    >
            ...
            >
                <div class="relative">
                    ...
                    <div class="absolute end-1 top-1">
                        <flux:modal.close>
                            <flux:button variant="subtle" icon="x-mark" size="xs" aria-label="Close modal" />
                        </flux:modal.close>
                    </div>
        ...
        @else
            ...
            <div class="absolute end-0 top-0 me-4 mt-4">
                <flux:modal.close>
                    <flux:button variant="subtle" icon="x-mark" size="sm" aria-label="Close modal" />
                </flux:modal.close>
            </div>
