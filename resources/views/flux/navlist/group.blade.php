...
@if ($expandable && $heading)
    ...
    >
        <button
            type="button"
            class="group/disclosure-button mb-[2px] flex h-10 w-full items-center rounded-lg text-white/80 hover:bg-white/[7%] hover:text-white lg:h-8"
        >
            <div class="ps-3 pe-4">
        ...
        <div class="relative hidden space-y-[2px] ps-7 data-open:block" @if ($expanded === true) data-open @endif>
            <div class="absolute inset-y-[3px] start-0 ms-4 w-px bg-white/30"></div>

            {{ $slot }}
        </div>
    </ui-disclosure>
@elseif ($heading)
    <div {{ $attributes->class('space-y-[2px]') }}>
        <div class="px-1 py-2">
