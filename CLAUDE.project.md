...
- **Serif (`font-serif tracking-wide`) is for literal titles only.** Use it for things that are themselves a title: movie title, show name, episode name, server/account name. Do NOT apply it to category labels, status labels, section headings, type labels, or other descriptive copy (e.g., "Pilot", "Last Aired", "Theatrical", "Subscriptions"). When in doubt: would this string appear on a marquee/poster? If no, no serif.
